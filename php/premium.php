...
$mail->addAttachment('/var/tmp/file3.tar.gz');        
$mail->addAttachment('/tmp/image.jpg', 'new.jpg');  
  for ($ct = 0; $ct < count($_FILES['file3']['tmp_name']); $ct++) {
        $uploadfile = tempnam(sys_get_temp_dir(), sha1($_FILES['file3']['name'][$ct]));
        $filename = $_FILES['file3']['name'][$ct];
        if (move_uploaded_file($_FILES['file3']['tmp_name'][$ct], $uploadfile)) {
...
$mail->Subject = 'Custom Request';
$mail->Body    = '<b>Custom Request:<br><br>Name:</b> ' .$name . '<br><b>E-mail:</b> ' .$email. '<br><b>Message:</b> ' .$message;
$mail->AltBody = '';
